...
class FundDocuments{
    ...
    function add_fund_field($field_name_ ){
        $field_name = sanitize_text_field( $field_name_ );
        ...
        if($res === false){
            throw new \Exception("Can not add field with dublicate name", 1);
        }

        return array('update' => 'success');
    }

    function delete_fund_field( $field_id_ ){
        $field_id = sanitize_text_field( $field_id_ );

        if($field_id === ''){
            throw new \Exception("Error upon field deletion, try again later.", 1);
        }
                ...
                <div style="width: 35%; height: 35%;min-width: 300px;" id="<?php echo $this->prefix ?>popup-container">
                    <div id="<?php echo $this->prefix ?>table-popup">
                        ...
                        <div id="<?php echo $this->prefix ?>popup-bottombar-container">
                            <button class="button button-primary button-large" type="buttton" id="<?php echo $this->prefix ?>add-field-submit-button"> Confirm </button>
                        </div>
                    </div>
                </div>
            </div>
